...
static void* _ddraw_bmp_thread_proc(void *param)
{
    ...
    if (!create) goto done;
    create(NULL, (void**)&s_lpDirectDraw, NULL);
...
static void _gdi_bmp_lock(void *pb, int x1, int y1, int x2, int y2)
{
    ((BMP*)pb)->reserved[0] = x1;
    ((BMP*)pb)->reserved[1] = y1;
    ((BMP*)pb)->reserved[2] = x2;
    ((BMP*)pb)->reserved[3] = y2;
}
...
{
    if (g_ffrge_hwnd) ShowWindow(g_ffrge_hwnd, SW_SHOW);
